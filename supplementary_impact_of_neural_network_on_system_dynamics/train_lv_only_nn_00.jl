#= 
Script to run the training of the Lotka Volterra UDE model n the dataset with error level 0.0.
=#
...
function train_uode_model(seed, iterator)
  ...
  function callback(θ, l, training_epochs, training_costs, best_training_parameters, training_parameterizations, epoch_parameterizations)
    ...
    if epoch % 100 == 1
      ...
      plt = Plots.plot(solutions, layout=2)
      Plots.scatter!(plt, solution_dataframe_to_plot.t, Array(solution_dataframe_to_plot[:, 2:end]), layout=2)
      Plots.scatter!(plt, validation_solution_dataframe_to_plot.t, Array(validation_solution_dataframe_to_plot[:, 2:end]), color= "green", layout=2)

      title!(plt, "Epoch " * string(epoch) * " -- cost: " * string(l) * "")
      display(plt)
   
    end

    #max 2 hours for an optimization
    if time() - initial_time > 60 * 2 * 60
